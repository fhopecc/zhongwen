...
def 辨詞(行, 游標位置=None):
    '''
    一、辨識字串中的數字、日期、URL、檔案路徑等詞項，並傳回其類與起迄位置。
    二、若指定游標位置，則僅傳回該位置下之詞項，無則傳回 None。
    三、類、詞、起、迄為結果項。
    '''
...
def 審核意見轉通知(意見:str) -> str:
    import re

    pat = r'(.+)函為((.+)銷毀(.+)一案)，謹擬具處理意見如說明.，簽請核示。'
    if m:=re.match(pat, 意見):
        通知 = f'貴{m[1][-1]}函為{m[2]}，其中除有關債權債務憑證及因案（如監察、審計、檢察、調查、廉政或稅務等機關調查中、法院審理中，或採購履約爭議處理中）應續予保存者外，其餘本室同意辦理，惟請注意檔案法及相關法令之規定，請查照。'
        return 通知

    pat = r'(.+)函為(.+)，擬予報損一案，謹擬具處理意見如說明二，簽請核示。'
    if m:=re.match(pat, 意見):
        通知 = f'貴{m[1][-1]}函為{m[2]}，擬予報損一案，既經貴府查明符合規定同意辦理，本室已予備查，請查照。'
        return 通知

    pat = r'(.+)聲復(.+)一案，謹擬具處理意見如說明二，簽請鑒核。'
    if m:=re.match(pat, 意見):
        通知 = f'貴{m[1][-1]}聲復{m[2].replace('該', '貴')}一案，核復如說明二，請查照。'
        return 通知
